...
class InducedSetAttentionBlock(nn.Module):
    ...
    def forward(self, query: torch.Tensor, points: torch.Tensor) -> torch.Tensor:
        ...
        # query, key, value
        # h, _ = self.att1(query.transpose(0,1),points.transpose(0,1),points.transpose(0,1))
        h, _ = self.att1(points.transpose(0,1),query.transpose(0,1),query.transpose(0,1))
        h = h.transpose(0,1)
        h = self.norm1_1(h+points)
        # print('h shape is', h.shape)  
        element_emb = self.norm2_1(self.mlp_1(h)+h)
        # print('set_emb shape is', set_emb.shape)  
        # set_emb shape is torch.Size([2, 1, 512])

        # induced bias
        h, _ = self.att2(query.transpose(0,1),element_emb.transpose(0,1),element_emb.transpose(0,1))
        h = h.transpose(0,1)
        ...
        # print('set_pred shape is', set_pred.shape) 
...
class MultiheadAttentionBlock(nn.Module):
    ...
    def forward(self, query: torch.Tensor, points: torch.Tensor) -> torch.Tensor:
        ...
        # print('query shape is', query.shape)  
        # print('point shape is', points.shape) 
        h, _ = self.att(query.transpose(0,1),points.transpose(0,1),points.transpose(0,1))
        h = h.transpose(0,1)
...
class RefSetTransHead(nn.Module):
    ...
    def __init__(
        ...
        #         nn.Linear(self.txt_size, self.hidden_size),
        #         nn.ReLU(True),
        #         nn.Linear(self.hidden_size, self.hidden_size),
        #     )
        assert compare_type in self.valid_compare_types
        self.compare_type = compare_type
        if self.compare_type == "bilinear": # bilinear proposals and references, devided by 8 due to memory issue
            self.compare = nn.Bilinear(self.hidden_size, self.hidden_size, self.hidden_size//2)
        elif self.compare_type == "token":
            self.compare = nn.Embedding(2,self.hidden_size) # take token as with extra embedding
            ...
            self.compare = None # use natural attention block (not self attention anymore!)
        else:
        ...
        if set_trans_type == "mab":
            if self.compare_type == "bilinear":
                self.set_trans = MultiheadAttentionBlock(embed_dim=self.hidden_size//2, num_heads=4,dropout=trans_dropout)
            elif self.compare_type == "2head" or self.compare_type == "att" :
                self.set_trans = MultiheadAttentionBlock(embed_dim=self.hidden_size, num_heads=4,dropout=trans_dropout)    
            ...
            if self.compare_type == "bilinear":
                ...
                self.set_trans = MultiheadAttentionBlock(embed_dim=self.hidden_size*2, num_heads=4,dropout=trans_dropout)    
                self.globe = nn.Embedding(1,self.hidden_size*2)
            elif self.compare_type == "att": # one head classify global existance by att
                self.set_trans = MultiheadAttentionBlock(embed_dim=self.hidden_size, num_heads=4,dropout=trans_dropout)    
                self.compare = nn.Bilinear(self.hidden_size, self.hidden_size, 1)
            else:
                raise NotImplementedError
            # elif self.compare_type == "2head":
            #     self.set_trans = MultiheadAttentionBlock(embed_dim=self.hidden_size, num_heads=4)    
            # else: # both token and concat will double the feature dim
            #     self.set_trans = MultiheadAttentionBlock(embed_dim=self.hidden_size*2, num_heads=4)
            # global embedding
            
        elif set_trans_type == "isab":
            # basically the same 
            if self.compare_type == "bilinear":
                self.set_trans = InducedSetAttentionBlock(embed_dim=self.hidden_size//2, num_heads=4,dropout=trans_dropout)
                self.globe = nn.Embedding(1,self.hidden_size//2)
            elif self.compare_type == "att":
                self.set_trans = InducedSetAttentionBlock(embed_dim=self.hidden_size, num_heads=4,dropout=trans_dropout)
    ...
    def forward(
        ...
        # print("inputs sizes are", xp.shape, yp.shape)
        ...
        if self.compare_type == "bilinear": # bilinear proposals and references
            ...
            if self.set_trans_type == "isab":
                ...
                output = self.set_trans(query=xp,points=yp) # Note: checked the shape, good
                # raise NotImplementedError
        else:
            raise NotImplementedError
    ...
    def losses(
        ...
        if self.set_trans_type == "mab_global" or self.set_trans_type == 'isab':
            ...
            frame_label = [torch.min(gt_class) for gt_class in gt_classes] # 0 means positive, 1 means negative
            
            loss_frame = binary_cross_entropy_with_logits(
                torch.stack(frame_prediction).squeeze(), 
                torch.stack(frame_label), 
                reduction="mean", flip_class=True
            )
    ...
    def inference(self, predictions: List[torch.Tensor], proposals: List[Instances]):
        ...
        if self.set_trans_type == "mab_global" or self.set_trans_type == 'isab':
            ...
            predictions = [prediction[1:] for prediction in predictions]
        # print(boxes[0].shape, predictions)    
        if len(predictions[0]) < len(proposals[0]): # extend predictions
            pad_right = len(proposals[0]) - len(predictions[0])
            # pad 0 to the right
            predictions = [F.pad(p, (0,0,0,pad_right), mode='constant',value=-10.) for p in predictions]
        # print(boxes[0].shape, predictions)  
...
class SetTransROIHeads(ROIHeads):
    ...
    def forward(
        ...
        if self.training:
            ...
            # random gt removal
            # for proposals_img in proposals: 
            for img_idx in range(len(proposals)):
                proposals_img = proposals[img_idx]
                if gt_rm_batch[img_idx]:
                    proposals_img_gt_rm_idx = proposals_img.gt_classes==1 # 1 means negative!!
        ...
        if self.training:
            ...
            pred_instances = self._forward_siam(features, ref_features, proposals, return_top_feature)
    ...
    def _forward_siam(
        ...
        # print('siam_features:', [x.shape for x in siam_features])
        ...
        if self.training:
            ...
            pred_instances,  kept_indices = self.siam_head.inference(predictions, proposals)
            if not return_top_feature:
                return pred_instances

            # compute frame feature: concate proposal with reference
            top1_feature_pool = [ F.adaptive_avg_pool2d(feat[idx[0]],(1,1)).squeeze() \
                    for feat,idx in zip(siam_features,kept_indices)]
            reference_feature_pool = [ F.adaptive_avg_pool2d(feat,(1,1)).squeeze() for feat in ref_features]
            frame_features = [torch.cat([prop,ref]) for prop,ref in zip(top1_feature_pool, reference_feature_pool)]
            # TODO optimise, for now I hack the instance class

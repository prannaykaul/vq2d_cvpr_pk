...
def evaluate_vq(annotations, cfg, device_id, use_tqdm=False):
    ...
    for idx, annotation in enumerate(annotations_iter):
        ...
        # pdb.set_trace()
        # want to keep the first detection associated with each peak
        if len(peaks) > 0:
            peak_bboxs = [ret_bboxes[peak][0] for peak in peaks]
            # peak_frames = [clip_frames[init_state.fno] for init_state in peak_bboxs]
            peak_scores = [ret_scores[peak][0] for peak in peaks]
        else:
            peak_bboxs = [ret_bboxes[-1][0]]
            # peak_frames = [clip_frames[peak_bboxs[0].fno]]
            peak_scores = [ret_scores[-1][0]]
        ...
        # add peak detections
        pred_peak_bboxes.append(peak_bboxs)
        pred_peak_scores.append(peak_scores)
        # pred_peak_frames.append(peak_frames)
        pred_peaks.append(peaks)
...
def main(cfg: DictConfig) -> None:
    ...
    # if cfg.data.debug_mode:
        # output = evaluate_vq(annotations, cfg, device_id=0, use_tqdm=False)
    # import pdb; pdb.set_trace()
    # print(torch.cuda.device_count())
    ...
    predictions = {
        ...
        "predicted_bboxes": [
            [bb.to_json() for bb in bbs]
            for bbs in predictions["predicted_bboxes"]
        ],
